...
def get_bot_response(str_access_token, ask_data, type, botsession):
    ...
    print('\n小蜜:', action['say'],'confidence:',action['confidence'],'type:',action['type'])
    return result
...
def choose_option(result):
    ...
    if(option_list != []):
        ...
        while True:
            try:
                option_result =int(input('Choose your answer(1~%d): ' %(len(option_list))))
                if option_result >= 1 and option_result <= len(option_list):
                    break
                else: print('please input in the range(1~%d): ' %len(option_list),end = ' ')   #false handle
            except ValueError: print('please input integer: ',end = ' ')                       #false handle
...
if __name__ == '__main__':
    ...
    while True:
        ...
        #    data_type = "EVENT"
        if data == 'q': break
        response = get_bot_response(access_token, data,data_type,lastbotsession)
